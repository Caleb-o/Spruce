...
use crate::sprucetype::SpruceType;
...
impl SymTable {
    ...
    pub fn new_local(&mut self, identifier: Span, mutable: bool, kind: Box<SpruceType>) {
    ...
    pub fn find_local(&self, source: &Source, span: &Span, anyscope: bool) -> Option<&Local> {
        for local in self.locals.iter().rev() {
            if anyscope && (local.depth == GLOBAL_DEPTH || local.depth >= self.depth_limit) {
                if local.identifier.compare(source, span) {
                    return Some(local);
                }
            }
        }
    ...
    fn find_local_count(&self) -> u16 {
        self.locals.iter().rev().fold(0, |acc, l| {
