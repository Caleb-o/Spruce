...
impl Ast {
    ...
    pub fn new_struct_literal(
        token: Token,
        identifier: Token,
        values: Vec<(Token, Option<Box<Ast>>)>,
    ) -> Box<Self> {
        Box::new(Self {
            token,
    ...
    pub fn new_error_or_value(which: ErrorOrValue, expression: Box<Ast>) -> Box<Self> {
        Box::new(Self {
            token: expression.token.clone(),
    ...
    pub fn new_parameter(token: Token, type_signature: Box<Ast>) -> Box<Self> {
        Box::new(Self {
            token,
    ...
    pub fn new_payload(token: Token, expression: Box<Ast>) -> Box<Self> {
        Box::new(Self {
            token,
    ...
    pub fn new_raw(token: Token, returns: Option<Box<Ast>>, code: Vec<Box<Ast>>) -> Box<Self> {
        Box::new(Self {
            token,
    ...
    pub fn new_do_while_statement(token: Token, body: Box<Ast>, condition: Box<Ast>) -> Box<Self> {
        Box::new(Self {
            token,
